...
## 动机
此技能必须严格跟在 `interview-alignment` 之后使用。它的职责**不是**与用户闲聊出题，它是一个冷酷无情的单纯“计划生成工厂”。负责将策略文档直接转化为可执行的清单。
...
> "为您量身定制的单兵作战演练计划已经写入了对应文件夹的 `prep-plan.md` 中。现在万事俱备，我们可以开启真正的拉练了。正在帮您呼唤 `@[/interview-mocking]` 执行器。"
